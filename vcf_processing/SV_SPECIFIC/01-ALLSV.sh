...
bcftools merge -l samples.txt -O v | sed 's/hereford.//g' | vcfbreakmulti | python SVLEN.py - 500 | bgzip -c > JOINT/JOINED.SV500.vcf.gz

# Get all indels
bcftools merge -l samples.txt -O v | sed 's/hereford.//g' | vcfbreakmulti | bcftools view -v indels -O z -o JOINT/JOINED.indels.vcf.gz
