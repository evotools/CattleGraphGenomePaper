...
for ( var_type in unique(mydata$TYPE) ){
    ...
    for ( n in seq(1, nrow( tmp_files ) ) ){
        tmp_VARS = read_tsv(as.character(tmp_files[n,] %>% pull(FULL_PATH)), col_types = cols(GROUP = col_character()) ) %>%
                    ...
                    select( ALGORITHM, GROUP, NV )
        if (!exists("allVARS")){
            allVARS = tmp_VARS
        } else {
            allVARS = rbind(allVARS, tmp_VARS)
        }
        cat("Done: ", as.character(tmp_files[n,] %>% pull(FULL_PATH)), "\n")
    }
    summaries = allVARS %>% group_by(ALGORITHM, GROUP) %>% summarise(NV=n())
