...
for ( var_type in unique(mydata$TYPE) ){
    ...
    for ( n in seq(1, nrow( tmp_files ) ) ){
        dat[[n]] = read_tsv(as.character(tmp_files[n,] %>% pull(FULL_PATH)), col_types = cols(GROUP = col_character(), DP= col_double()) ) %>%
                    mutate(ALGORITHM = as.character(tmp_files[n,] %>% pull(FLDR)), SAMPLE = as.character(tmp_files[n,] %>% pull(SAMPLE)) ) %>%
                    select( ALGORITHM, GROUP, DP )
        cat("Done: ", as.character(tmp_files[n,] %>% pull(FULL_PATH)), "\n")
    }
    dat<-bind_rows(dat, .id="Index")
    summaries = dat %>% group_by(ALGORITHM, GROUP) %>% summarise(DP=median(DP))
    summaries$GROUP = factor(summaries$GROUP, level = classVec(-60, 60))
    write.csv2(summaries, paste("SUMMARIES/DP_WGS",var_type,".csv", sep = ""))
    p = summaries %>% 
            ggplot(aes(x=GROUP, y=DP, color = ALGORITHM)) +
    ...
    ggsave(paste("PLOTS/DP_WGS_",var_type,".pdf", sep = ""), plot = p,device = "pdf", height = 9, width = 16)
    rm(dat)
    gc()
}