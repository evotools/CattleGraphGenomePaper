...
for ( var_type in unique(mydata$TYPE) ){
    ...
    for ( n in seq(1, nrow( tmp_files ) ) ){
        tmp_DP = read_tsv(as.character(tmp_files[n,] %>% pull(FULL_PATH)), col_types = cols(GROUP = col_character()) ) %>%
        ...
        if (!exists("allDPs")){
            ...
            allDPs = rbind(allDPs, tmp_DP)
        }
        cat("Done: ", as.character(tmp_files[n,] %>% pull(FULL_PATH)), "\n")
    }
    summaries = allDPs %>% group_by(ALGORITHM, GROUP) %>% summarise(DP=median(DP))
