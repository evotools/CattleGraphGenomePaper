...
bcftools view -O v -f .,PASS -i 'GT!="RR" & INFO/DP>=15 & INFO/DP<=120' $vcf | \
    vcftools --vcf - --TsTv-summary --out SOFT_DEPTH_FILT/${outfld}_ALL/$sample/${sample}_ALL
    
bcftools view -O v -f .,PASS -i 'GT!="RR" & INFO/DP>=15 & INFO/DP<=120' $tgtfld/$sample/isec_output_$sample/0000.vcf.gz | \
    vcftools --vcf - --TsTv-summary --out SOFT_DEPTH_FILT/${outfld}_NOVEL/$sample/${sample}_NOVEL

bcftools view -O v -f .,PASS -i 'GT!="RR" & INFO/DP>=15 & INFO/DP<=120' $tgtfld/$sample/isec_output_$sample/0002.vcf.gz | \
    vcftools --vcf - --TsTv-summary --out SOFT_DEPTH_FILT/${outfld}_KNOWN/$sample/${sample}_KNOWN 



bcftools view -O v -f .,PASS -i 'GT!="RR" & INFO/DP>=15 & INFO/DP<=120 & QUAL>30' $vcf | \
    vcftools --vcf - --TsTv-summary --out SOFT_DEPTH_QUAL_FILT/${outfld}_ALL/$sample/${sample}_ALL
    
bcftools view -O v -f .,PASS -i 'GT!="RR" & INFO/DP>=15 & INFO/DP<=120 & QUAL>30' $tgtfld/$sample/isec_output_$sample/0000.vcf.gz | \
    vcftools --vcf - --TsTv-summary --out SOFT_DEPTH_QUAL_FILT/${outfld}_NOVEL/$sample/${sample}_NOVEL

bcftools view -O v -f .,PASS -i 'GT!="RR" & INFO/DP>=15 & INFO/DP<=120 & QUAL>30' $tgtfld/$sample/isec_output_$sample/0002.vcf.gz | \
