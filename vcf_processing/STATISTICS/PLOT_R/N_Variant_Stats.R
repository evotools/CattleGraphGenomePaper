...
makeValues <- function(samples, sample_dirs, algorithm){
  ...
  for (n in seq(1, nrow(samples))){
    ...
    dataset = dataset %>% select(GROUP, avgQUAL, N)
    colnames(dataset)[2] = paste(sample, "QUAL", sep = "_")
  ...
  values["MEAN_QUAL"] = rowMeans(values[,seq(2, ncol(values), 2)], na.rm=TRUE) 
  values["MEAN_Nv"] = rowMeans(values[,seq(3, ncol(values), 2)], na.rm=TRUE) 
  values = values %>% select(GROUP, MEAN_QUAL, MEAN_Nv)
  colnames(values)[c(2,3)] = c( 
    paste(gsub("./","", algorithm), "QUAL", sep = "_"), 
...
for (G in c("ALL", "NOVEL", "KNOWN") ){
  ...
  for (br in c("NDama", "Angus", "Sahiwal")){
    ...
    to_plot_tmp1 = melt(final[,c("GROUP", colnames(final)[grepl("_QUAL", colnames(final))] )], id.vars = "GROUP") %>% 
      select("Length" = GROUP, "QUAL_zscore"=value, "Algorithm"=variable)
    to_plot_tmp1$Algorithm = gsub("_QUAL","", to_plot_tmp1$Algorithm)
    ...
    p = to_plot %>% 
      ggplot(aes(x=Length, y=QUAL_zscore, color = Algorithm)) +
      geom_point() + geom_line(aes(group = Algorithm)) + 
      scale_y_continuous() +
      theme(axis.text.x = element_text(angle=90, hjust=1)) +
      labs(x="Length", y="QUAL (avg Z-score)", color = "Software", title = paste("QUAL of variant by variant size and number")) 
    ggsave(paste("PLOTS/", G, "/Number_Variants_", br,"_",G,".pdf", sep = ""), plot = p,device = "pdf", height = 9, width = 16)
  }
  ...
  p = to_plot %>% 
    ...
    theme(axis.text.x = element_text(angle=90, hjust=1)) +
    labs(x="Length", y="QUAL (avg Z-score)", color = "Software", title = paste("QUAL of variant by variant size and number")) 
  ggsave(paste("PLOTS/", G, "/Number_Variants_WGS_",G,".pdf", sep = ""), plot = p,device = "pdf", height = 9, width = 16)
}
