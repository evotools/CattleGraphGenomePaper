...
for ( var_type in unique(mydata$TYPE) ){
    ...
    for ( n in seq(1, nrow( tmp_files ) ) ){
        tmp_AB = read_tsv(as.character(tmp_files[n,] %>% pull(FULL_PATH)), col_types = cols(GROUP = col_character()) ) %>%
    ...
    summaries = allABs %>% group_by(ALGORITHM, GROUP) %>% summarise(AB=median(AB))
    ...
    ggsave(paste("PLOTS/AB_WGS",var_type,".pdf", sep = ""), plot = p,device = "pdf", height = 9, width = 16)
    rm(allABs)
    gc()
}