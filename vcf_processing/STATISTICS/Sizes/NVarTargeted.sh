...
cd ../../
#filt='GT="het"'
#filt='GT="het" && FORMAT/DP>=15 && FORMAT/DP<=120'
#filt='GT="het" && FORMAT/DP>=15 && FORMAT/DP<=120 & QUAL>=30'
bcftools view -f .,PASS -i 'GT!="RR"' $vcf | \
    bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t[%GT\t%DP\t%AD]\n' | \
	python ./NBySizeByQUAL.py -i - | bgzip -c > ${outfld}_ALL/$sample/${sample}_AllelicBalanceAllSites.tmp.gz

bcftools view -f .,PASS -i 'GT!="RR"' $tgtfld/$sample/isec_output_$sample/0000.vcf.gz | \
    bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t[%GT\t%DP\t%AD]\n' |\
        python ./NBySizeByQUAL.py -i - | bgzip -c > ${outfld}_NOVEL/$sample/${sample}_AllelicBalanceNovelSites.tmp.gz

bcftools view -f .,PASS -i 'GT!="RR"' $tgtfld/$sample/isec_output_$sample/0002.vcf.gz | \
    bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t[%GT\t%DP\t%AD]\n' |\
        python ./NBySizeByQUAL.py -i - | bgzip -c > ${outfld}_KNOWN/$sample/${sample}_AllelicBalanceKnownSites.tmp.gz
